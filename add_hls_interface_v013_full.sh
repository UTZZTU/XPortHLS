...
echo "[1/7] Add HLS Interface Extractor v1"

mkdir -p xporthls/realrepo
touch xporthls/realrepo/__init__.py

cat > xporthls/realrepo/hls_interface_extractor.py <<'EOT'
...
echo "[2/7] Add HLS Interface IR validator"

cat > xporthls/realrepo/validate_hls_interface_v013.py <<'EOT'
from __future__ import annotations
...
echo "[3/7] Add v0.0.13 orchestration runner"

cat > xporthls/realrepo/run_hls_interface_v013.py <<'EOT'
from __future__ import annotations
...
echo "[4/7] Update compatibility profiler recommendation after HLS extraction"
...
echo "[5/7] Update README with HLS interface extraction usage"
...
if "## HLS interface extraction" not in text:
    text = text.rstrip() + "\n\n" + section.strip() + "\n"

p.write_text(text, encoding="utf-8")
PY

echo "[6/7] Create v0.0.13 replay script"

cat > add_hls_interface_v013_replay.sh <<'EOT'
#!/usr/bin/env bash
set -e
...
chmod +x add_hls_interface_v013_replay.sh

echo "[7/7] Run v0.0.13 replay"

./add_hls_interface_v013_replay.sh

echo "[v0.0.13] Git status"
